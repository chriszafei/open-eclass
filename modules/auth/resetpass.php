...
if (isset($_POST) && !empty($_POST) ) {
	file_put_contents($webDir . '/user_post_log.log', date( '[d-m-Y H:i:s]') . " Request saved\n". print_r($_POST, true) . "\n", FILE_APPEND | LOCK_EX);
...
if (isset($_REQUEST['do']) && $_REQUEST['do'] == "go") {
	...
	$tool_content .= '<p>Εισάγετε το όνομα χρήστη και τον κωδικό σας, ώστε να ενεργοποιήσουμε ξανά το λογαριασμό σας.</p>';
	...
	$tool_content .= "<form method=\"post\" action=\"".$REQUEST_URI."\">
		...
		<td>
		<input type=\"text\" name=\"userName\" size=\"40\" value=\"$request_username\"/>
		</td>
		<tr>
		<th>$langPassword: </th>
		<td>
